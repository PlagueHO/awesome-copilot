...
### Automatically Updating Marketplace Plugins

*(v1.0.79+)* Set `"autoUpdate": true` on an `extraKnownMarketplaces` entry to automatically update its plugins to the latest version at the start of every session:
...
This is useful when you want a team-shared marketplace to always be current without requiring everyone to remember to run an update command manually. First-party plugins (the ones bundled with Copilot CLI and the Copilot app) already auto-update to the latest version at session start by default.
